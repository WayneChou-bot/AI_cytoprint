...
那個 9.0 % 是**對 113 個難度天差地遠的 MoA 類別取平均**的結果,而且它在兩個方向上都誤導:
**102 個類別是 0 % Top-1**,但另有一小群接近滿分——而且**全部 Top-1 命中有 59 % 只來自 5 個類別**。
把分布攤開,比引用它的平均值有用得多:

| MoA 類別 | n | Top-1 | Top-5 | mAP |
|---|---|---|---|---|
| Aurora 激酶抑制劑 | 5 | **100 %** | 100 % | 0.834 |
| HDAC 抑制劑 | 3 | **100 %** | 100 % | 0.783 |
| mTOR 抑制劑 | 9 | **89 %** | 89 % | 0.728 |
| 糖皮質素受體促效劑 | 9 | **78 %** | 89 % | 0.641 |
| JAK 抑制劑 | 5 | 40 % | 40 % | 0.110 |
| 蛋白酶體抑制劑 | 3 | 33 % | **100 %** | 0.583 |
| 拓樸異構酶抑制劑 | 7 | 29 % | 43 % | 0.086 |
| EGFR 抑制劑 | 6 | 17 % | 50 % | 0.080 |
| 環氧合酶抑制劑 | 14 | 7 % | 21 % | 0.050 |
| 血清素／腎上腺素／組織胺／多巴胺受體配體 | 7–13 | **0 %** | 0–29 % | ≤ 0.04 |
| 磷酸二酯酶抑制劑 | 11 | **0 %** | 9 % | 0.026 |
...
與訊號調節劑,它們在 A549 肺上皮細胞上不產生可見表型——該受體甚至可能根本不表現。
**沒有表型,就沒有形態可以指紋化**;這是檢測方法的邊界,不是演算法的失敗。

但「受體配體都不行」會是錯的結論:**糖皮質素受體促效劑也是受體配體,Top-1 卻有 78 %**,
因為 A549 確實表現 GR,而且其下游是強烈的轉錄重編程。判準不是藥物的標靶類別,
而是**這個細胞株對它有沒有反應**。

兩個告誡。兩種平均都有報——逐類別平均(macro)mAP = **0.073**,依查詢數加權 = **0.091**
——因為只報一個等於選了一個對自己有利的定義。另外 113 個類別中有 57 個只含 2 個化合物,
此時 Top-1 只能是 0 % 或 100 %;互動圖因此預設過濾為 n ≥ 5,真正可信的是那些 n ≥ 5
**且** Top-1 高的類別(Aurora、mTOR、GR 促效劑),而不是任何單一的 2 化合物類別。

`test_consistency.py` 會從已發布的鄰居表重新計算每個類別的比率,並斷言它們加權平均回頁首的 Top-1。
...
Harmony 近乎完美的批次混合(3.17 → 1.04),伴隨的是時間點的條件結構**幾乎被完全抹平**(1.28 → 1.02),
