...
| 層 | 做什麼 | 規模 |
|---|---|---|
| **1. 影像 → 指紋**(`images/`) | 真實五通道影像 → 自建細胞核分割 → 59 個形態特徵 → 對 DMSO 標準化 | 9 個化合物、701 顆細胞 |
| **2. Profile 分析**(`pipeline.py`) | 已算好的 CellProfiler profiles → 清理 → 共識指紋 → UMAP、MoA 檢索、候選活性篩選、去偏比較 | 19,200 wells、561 化合物、587 特徵 |
| **3. 嚴謹評估**(`mvp/`) | 批次校正到底有沒有效?雙指標 + plate-block bootstrap + 配對 ΔCI | LINCS(50 盤)與真實 JUMP-Target(16 盤、4,102 wells) |
...
NAD⁺ 耗竭 → 凋亡染色質濃縮),而弱表型的對照藥幾乎不動。

指紋也**不只是「死了多少細胞」的代理**:表型強度與密度變化只有中度相關
(Pearson r = +0.63,p = 0.09,n = 8);dexamethasone 細胞損失很少卻排第 3,其特徵由
**粒線體訊號的特異性下降**主導(Cohen's d = −4.09),與醣皮質素受體作用於粒線體的文獻一致。
...
真實 JUMP-Target 結果(`mvp/jump_mvp_results.json`,4,102 wells,block = plate):
...
在 JUMP 上校正明顯移除了批次結構(3.17 → 1.04,幾乎完全混合)且**沒有可測的生物代價**——
所有配對 Δ 的信賴區間都跨 0。同一引擎在單一中心的 LINCS pilot 上效果弱得多,符合預期。
兩者都**沒有**被誇大成顯著。
...
- 影像層**每個化合物只有 1 個 field、無實驗重複**——機制表為案例佐證,不是統計結果。
